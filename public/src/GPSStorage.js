export class GPSStorage {
    ...
    // Adds satellite data and calculates the distance
    addSateliteData(data) {
      ...
      // Calculate distance based on the time delay
      ...
      // Set a timeout to remove satellite data after the storage time
    ...
    // Calculates the object's position using satellite data
    getObjectPosition() {
    ...
    // Prepares satellite data and selects the latest three points
    prepareDots() {
      const sateliteArray = Array.from(this.sateliteData.values());
  
      // Sorts satellites by their added time (latest first)
      sateliteArray.sort((a, b) => b.addedAt - a.addedAt);
  
      // Selects the latest three satellites
